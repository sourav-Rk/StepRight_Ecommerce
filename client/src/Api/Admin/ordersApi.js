...
export const getAllOrders = async (options = {}) => {
    try {
        const response = await axiosInstance.get('/admin/orders',options);
...
//API call to update the payment status of cod
export const updatePaymentCod = async(orderId) => {
    try{
        const response = await axiosInstance.patch('/admin/orders/updatePaymentCod',{orderId});
        return response.data
    }
    catch(error){
        throw error?.response?.data || error
