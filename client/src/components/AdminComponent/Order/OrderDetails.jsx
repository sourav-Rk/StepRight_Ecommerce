...
const OrderDetails = () => {
  ...
  //to handle approve payment cod
  const handleApprovePayment = async (orderId) => {
    try {
      const resonse = await updatePaymentCod(orderId);
      message.success(resonse.message);
      await fetchOrder();
    } catch (error) {
      message.error(error?.message || "error in approving refund");
    }
  };

  const confirmApprovePayment = (orderId) => {
    Modal.confirm({
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="space-y-2">
                ...
                <p>
                  <strong>Ordered Date:</strong> {formatDate(order.createdAt)}
                </p>
              </div>
              <div className="space-y-2">
                  ...
                  >
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="mt-2 sm:mt-0 flex items-center space-x-3">
                          ...
                          item.refundStatus !== "None" && (
                            <>
                              {item.refundStatus !== "Approved" ? (
                                <button
                                  onClick={() =>
                                    confirmRefund(orderId, item._id)
                                  }
                        ...
                        {/* Payment Approval Button or Payment Received Badge */}
