...
export const addProduct = async (req, res) => {
  try {
    const { name, description, category, brand, offer, images, variants } =
      req.body;
    ...
    if (offer && (typeof offer !== "number" || offer < 0)) {
      return res
        .status(400)
        .json({ message: "Offer must be a non-negative number" });
    }
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: `variant ${i + 1} is missing required fields` });
      }
      ...
      if (sizeSet.has(sizeKey)) {
        return res
          .status(400)
          .json({ message: `Duplicate variant size : ${variant.size}` });
      }
      sizeSet.add(sizeKey);
      ...
      if (Number(variant.regularPrice) < 0) {
        return res
          .status(400)
          .json({
            message: `Regular price for variant ${variant.size} cannot be negative`,
          });
      }
      if (Number(variant.quantity) < 0) {
        return res
          .status(400)
          .json({
            message: `Quantity for variant ${variant.size} cannot be negative`,
          });
      }
    }
...
export const blockProduct = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({
        message: `${product.name} has been ${product.isActive} ? "blocked" : "unblocked"`,
      });
  } catch (error) {
    return res
      .status(500)
