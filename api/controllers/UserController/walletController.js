...
export const deductWalletAmount = async(req,res,next) => {
    ...
    if(!amount || isNaN(amount) || amount<=0) return next(errorHandler(400,"Please provide a valid positive amount"));
    ...
    try{
        ...
        if(wallet.balance<amount) return next(errorHandler(400,"Insufficient wallet balance"));
        ...
        return next(errorHandler(500,"Something went wrong while deducting wallet balance"));
    }
}
 ...
 export const addMoneyToWallet = async (req,res,next) => {
   ...
   try{
      ...
      const walletUpdate = await walletDB.updateOne(
        { userId },
        { $push: { transactions: transactionDetails }, $inc: { balance: amount } },
        { upsert: true }
      );
