...
export const addAddress = async (req, res, next) => {
    ...
    try{
        ...
        return next(errorHandler(500,"something went wrong when adding new address"))
    }
}
...
export const editAddress = async (req, res, next) =>{
    ...
    try{
        ...
        if(updateAddress.modifiedCount===0) return res.status(400).json({message : "No changes were made"});
...
export const deleteAddress = async (req, res, next) =>{
    ...
    try{
        ...
        if(deleteAddress.deletedCount ===0 ) return next(errorHandler(404,"Address not found or already deleted"))
