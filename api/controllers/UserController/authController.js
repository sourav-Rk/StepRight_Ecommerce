...
export const generateAndSendOTP = async (req, res, next) => {
  let { email, firstName, lastName, phone, password, confirmPassword } =
    req.body;
  ...
  try {
    ...
    if (userExist)
      return next(
        errorHandler(409, "Email already exist.Please use a different Email")
      );
...
export const verifyOTPAndCreateUser = async (req, res) => {
  ...
  try {
    ...
    if (formData.referredBy) {
      const referrer = await usersDB.findOne({
      ...
      if (referrer) {
        ...
        const referrerWallet = await walletDB.findOne({
...
export const verifyLogin = async (req, res) => {
  ...
  try {
    const userExist = await usersDB.findOne({ email, role: "user" });
    ...
    generateAccessToken(res, userExist);
    generateRefreshToken(res, userExist);
...
export const googleAuth = async (req, res, next) => {
  ...
  try {
    ...
    if (UserExists) {
      generateAccessToken(res, UserExists);
      generateRefreshToken(res, UserExists);
      return res
        .status(200)
      ...
      generateAccessToken(res, newUserDetail);
      generateRefreshToken(res, newUserDetail);
      await walletDB.create({
        userId: newUser._id,
        balance: 0,
        transactions: [],
