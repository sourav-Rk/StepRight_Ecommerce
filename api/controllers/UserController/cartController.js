...
export const addToCart = async(req, res, next) => {
    try{
        ...
        if(!productId || !size) return next(errorHandler(400,"Product and size are required"));
        ...
        if(!variant) return next(errorHandler(404,"The selected size is not available for this product"));
        if(variant.quantity<1) return next(errorHandler(404,"The size is out of stock"));
        ...
        if(existingProduct){
            ...
            if(existingProduct.quantity<5){
                ...
                if(newQuantity>variant.quantity) return next(errorHandler(400,"cannot add more than available stock"));
    ...
    export const getCartProducts = async (req, res, next) =>{
        try{
            ...
            const cartProducts = await cartDB.findOne({userId}).populate("items.product");

            if(!cartProducts) return next(errorHandler(404,"You havent added any items to the cart"));
    ...
    export const updateCartItemQuantity = async (req,res,next) => {
        try{
            ...
            if(newQuantity<1) return next(errorHandler(400,"Quantity must be atleast one"));
            if(newQuantity>5) return next(errorHandler(400,"You can only buy a quantity of 5"));
            if (newQuantity> variant.quantity)  return next(errorHandler(400,"Requested quantity exceeds available stock"));
